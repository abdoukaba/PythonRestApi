...
I would be interesting to link the live game streaming with a weather to receive notification to know if the weather condition may impact the game, as bad weather can easily take down live stream games if unprepared.
To provide   for each live game a live stream of mixed event (game action,weather conditions changes) It would be interesting to use a cron job to check climates conditions.
